...
export function getCourses(name) {
  ...
  return async (dispatch) => {
    let response = await axios.get(
      `https://cursort-production-2088.up.railway.app/courses?name=${name}`
    );
    dispatch({
      type: GET_COURSES,
...
export function getDetail(id) {
  return async (dispatch) => {
    let response = await axios.get(`https://cursort-production-2088.up.railway.app/courses/${id}`);
...
export function getCategory(){
    return async (dispatch) =>{
        let categories = await axios.get('https://cursort-production-2088.up.railway.app/categories')
        return dispatch({
            type: GET_CATEGORIES,
...
export function getFilterCategory(id){
  ...
  if(id=== 'all'){
    return async (dispatch)=>{
      const response = await axios.get(`https://cursort-production-2088.up.railway.app/filter/category`)
      dispatch({
  ...
  return async (dispatch)=>{
    const response = await axios.get(`https://cursort-production-2088.up.railway.app/filter/category/?id=${id}`)
    dispatch({
      type:GET_FILTER_CATEGORY,
      payload : response.data //array de cursos
    })
...
export function createNewCategory(payload) {
  return async function (dispatch) {
    let json = await axios.post('https://cursort-production-2088.up.railway.app/categories', payload);
    return json;
  }
}
...
//   return async (dispatch)=>{
//     const response = await axios.get(`https://cursort-production-2088.up.railway.app/filter/?duration${duration.duration}`) // ==>no se como conseguir todos los query juntos 
